...
<div className="absolute inset-0 -z-10 h-full w-full bg-green-100 bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:14px_24px]">
      ...
      <div className="md:mycontainer p-2 min-h-[77.5vh]">
        ...
        <div className="flex flex-col p-4 text-black gap-8 items-center">
          ...
          <button onClick={savePassword} className="flex justify-center items-center gap-2 bg-green-400 hover:bg-green-300
           rounded-full px-8 py-2 w-fit border border-green-900 font-bold">
            <lord-icon
          ...
          <table className="table-auto w-full rounded-lg overflow-hidden mb-10">
            ...
            <tbody className="bg-green-100">
              {passwordArray.map((item, index)=>{
                ...
                <img className="cursor-pointer w-5 pt-2 pl-2" src="icons/copy.svg" alt="" onClick={()=>{copyText(item.site)}}/>
                </div>
                </td>
                <td className="py-2 border border-white text-center">
                <div className="flex justify-center items-center ">
                  <span>{item.username}</span>
                <img className="cursor-pointer w-5 pt-2 pl-2" src="icons/copy.svg" alt="" onClick={()=>{copyText(item.username)}}/>
                </div>
                </td>
                <td className="py-2 border border-white text-center">
                <div className="flex justify-center items-center ">
                  <span>{"*".repeat(item.password.length)}</span>
                <img className="copy cursor-pointer w-5 pt-2 pl-2" src="icons/copy.svg" alt="" onClick={()=>{copyText(item.password)}}/>
                </div>
                </td>
                <td className="py-2 border border-white text-center">
                  <div className="flex justify-center items-center">
                <span className="cursor-pointer mx-1 px-1" onClick={()=>{editPassword(item.id)}}>
                <img src="icons/edit.jpg" alt="" width={26}/>
                </span>
                <span className="cursor-pointer mx-1 px-1" onClick={()=>{deletePassword(item.id)}}>
                <img src="icons/delete.png" alt="" width={26} />
                </span>
                </div>
                </td>
              </tr>
